...
abstract: >
  ...
  target genes.
  Here, we provide sevenC, a tool that uses protein binding signals from ChIP-seq and
...
In this vignette, we show how to use the R package `r BiocStyle::Biocpkg("sevenC")` to predict chromatin looping interactions between CTCF motifs by using only ChIP-seq data form a single experiment. 
...
# get motifs pairs
gi <- prepareCisPairs(motifs, scoreColname = "sig")
```
...
Here, we use CTCF motif hits from RSAT matrix-scan on chromosome 22 as provided as `motif.hg19.CTCF.chr22` in the `r BiocStyle::Biocpkg("sevenC")` package. For more information on the motif data set, see `?motif.hg19.CTCF`.
```{r, results = "hide", message = FALSE}
library(sevenC)
...
Note, that the function `prepareCisPairs()` is wrapper for three individual functions that performs each step separately and allow more options. 
...
A more detailed evaluation of prediction performance using cross-validation and different cell types is soon available in the sevenC paper. 
